...
@modules_bp.route("/visits")
@login_required
def visits_list():
    page = request.args.get("page", 1, type=int)
    search = request.args.get("search", "")
    result = _get_records(seed.VISITS_MODULE_ID, page=page, page_size=20,
...
def _visits_form_context():
    """Fetch patients and doctors for dropdown selects."""
...
@modules_bp.route("/visits/create", methods=["GET", "POST"])
@login_required
def visits_create():
    if request.method == "POST":
        ...
        return redirect(url_for("modules.visits_list"))
    return render_template("modules/visits/form.html", visit=None, action="create",
                           **_visits_form_context())


@modules_bp.route("/visits/<record_id>")
@login_required
def visits_detail(record_id):
    resp = _get_record(seed.VISITS_MODULE_ID, record_id)
    if not resp.data:
...
@modules_bp.route("/visits/<record_id>/edit", methods=["GET", "POST"])
@login_required
def visits_edit(record_id):
    ...
    if request.method == "POST":
        ...
        return redirect(url_for("modules.visits_detail", record_id=record_id))
